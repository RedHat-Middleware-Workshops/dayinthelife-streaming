...
. Update the *application.properties* file under `support/projects/module-1/` so we can create a _config map_ that will provide the AMQ address space for our event-driven services.
+
image::images/application-properties-configmap.png[Properties ConfigMap, role="integr8ly-img-responsive"]
+
[IMPORTANT]
====
Replace the placeholder text with your actual hostname.
====

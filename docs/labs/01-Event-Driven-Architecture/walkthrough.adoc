...
.Fuse Online
****
* link:{fuse-user-url}[Fuse Online Console]
****

=== Access Fuse Online

. Access the link:{fuse-user-url}[Fuse Online Console] and login using your credentials.
...
*Excelent!* You are ready to test your new architecture.

[time=5]
[id="test-eda"]
== Testing the new architecture

Remeber what happens when a service is down in our _traditional_ REST architecture? We get an error telling us that we were not able to process the order. Let's check what happens when we introduce microservices decoupling using events.
...
. A notification will show you that your order was received and a almost inmediately another notificaction will pop up telling us that our order was already proceseed. This is the *inventory* service feedback.
+
...
*Excelent!* Our services are working.
...
. Get back to the link:{fuse-user-url}[Fuse Online Console]
...
[TIP]
====
This will simulate the _shortage_ of the inventory service. Wait a few seconds for the service to be stopped.
====

image::images/fuse-stopped.png[Scaled To Zero, role="integr8ly-img-responsive"]
--

. Get back to the web application page and try again to order some async *Strawberry* flavor. What happened?
+
--
...
. Let's get the *inventory* service back online. Get back to the link:{fuse-user-url}[Fuse Online Console]

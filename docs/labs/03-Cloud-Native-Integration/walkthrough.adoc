...
. Now that we've populated the database table with records, navigate back to the *Eclipse Che* window and open the `FleurDeLune/projects/module-3-inventory project`.  Examine the `Inventory.java` file.  At this point we need to create 3 Camel routes.
+
+
[source,java,subs="attributes+"]
----

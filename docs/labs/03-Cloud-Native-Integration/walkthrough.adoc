...
To provide better customer experience, Closely monitoring the harvest from marshmallows farms on the moon. Plus harvest batch shipping schedule, will allow Fleur de lune more proactive. And faster respond to any emergency that may impact their business.
...
(Skip this apart and go directly to create Topics if you have already done so in other labs.)
...
. The harvest data are now streaming into the topic, we will be grading the marshmallows according to their sizes. First we will store all the updated grading information in a relational database. And also streaming separately to the shipping departments.
+
...
. “Fleur de lune” is introducing a new AI system for more efficient shipping, due to the high inter-planet cost. The shipping department is now responsible to preparing all data into a in-memory data lake for the AI system.

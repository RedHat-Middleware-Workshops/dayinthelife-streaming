...
oc apply -f ~/Downloads/6872572_rhpds-secret.yaml -n {user-username}
oc secrets link default 6872572-rhpds-pull-secret --for=pull -n {user-username}

. Create a `KafkaConnectS2I` Kubernetes Custom Resource to define your connector.
+
[source,bash,subs="attributes"]
----
...
. Wait for the default build to finish and deploy the first pod.

. Start a new build but this time point it to download the debezium connector plugin for SQL Server.
+
[source,bash,subs="attributes"]
----
oc start-build debezium-connect --from-archive http://central.maven.org/maven2/io/debezium/debezium-connector-sqlserver/0.10.0.Beta2/debezium-connector-sqlserver-0.10.0.Beta2-plugin.tar.gz --follow --no-cache -n {user-username}
----
...
. Navigate to the link:http://www-shared-app-earth.{openshift-app-host}/[PHP app] in a new browser tab.
. Download to your local system the link:https://raw.githubusercontent.com/RedHatWorkshops/dayinthelife-streaming/master/support/module-1/earth-orders.csv[Earth Orders File].

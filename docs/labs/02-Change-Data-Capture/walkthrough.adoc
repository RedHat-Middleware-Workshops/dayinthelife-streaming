...
[source,bash,subs="attributes+"]
----
/opt/mssql-tools/bin/sqlcmd -S mssql-server-linux -U sa -P Password! -d InternationalDB
----

. Query the table to check the information is there:
+
[source,bash,subs="attributes+"]
----
1> select top 5 * from dbo.Orders where OrderUser='{user-username}'
2> go
----
